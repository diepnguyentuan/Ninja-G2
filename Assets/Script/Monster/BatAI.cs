...
public class BatAI : MonoBehaviour, IDamageable
{
    ...
    [Header("Attack Timing (QUAN TRỌNG)")]
    [Tooltip("Thời gian chờ từ lúc bắt đầu Animation đến lúc bật Hitbox (Giây)")]
    public float damageWindUpTime = 0.3f;
    ...
    void Start()
    {
        ...
        if (attackHitbox != null)
        {
            ...
            {
                Debug.LogError("❌ LỖI: GameObject 'AttackHitbox' thiếu BoxCollider2D!");
            }
        }
        else
        {
            Debug.LogError("❌ LỖI: Chưa kéo AttackHitbox vào ô script BatAI!");
        }
    ...
    void Die()
    {
        if (isDead) return; // Ngăn chặn gọi Die nhiều lần
        isDead = true;
        ...
        // 🎯 2. GỌI HỆ THỐNG PLAYERSTATS ĐỂ CỘNG 75 XP
        if (PlayerStats.instance != null)
        {
            PlayerStats.instance.GainExp(xpValue);
            Debug.Log($"Đã tiêu diệt Dơi! +{xpValue} XP");
        }
